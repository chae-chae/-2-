...
void addNewWord(List* lp, BOOL* bp, FILE* ddlp);
...
int main(int argc, const char * argv[]) {
    ...
    while (menusel != 5) {
        switch (menusel) {
            ...
            case 4: wordBook(&list, &bres, dicDotList); break;
            default: break;
        }
        menusel = menuScreen();
    }
...
void wordBook(List *lp, BOOL *bp, FILE *ddlp){
    int sel;
    //    clear(); // 리눅스해주기
    ...
    while (sel != 5) {
        switch (sel) {
            case 1: addNewFile(lp, bp, ddlp); break; //단어장 만드는 메뉴
            case 2: addNewWord(lp, bp, ddlp); break;
...
void addNewFile(List* lp, BOOL* bp, FILE* ddlp){
